...
func executeTerraform(feature Feature, env, stateFilePath string, backendConfig BackendConfig, backendTFVarsPath string) error {
	...
	initCmd := exec.Command("terraform", "init",
		...
		fmt.Sprintf("-backend-config=key=%s", feature.StateFile),
...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("usage: tfRunner <environment> [features]")
		os.Exit(1)
	}
	env := os.Args[1]
	var featureNames []string
	if len(os.Args) > 2 {
		featureNames = strings.Split(os.Args[2], ",")
	}
	...
	for _, feature := range features {
		if len(featureNames) == 0 || contains(featureNames, feature.Name) {
